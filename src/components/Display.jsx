    ...
    return (
        <div className="fixed z-10 top-0 left-0 bg-gradient-to-r from-[#FF5722] to-[#F97316] w-full h-full lg:w-[430px] ">
            ...
            <div className="absolute top-[85px] w-full lg:w-[430px] flex flex-col items-center">
                {options?.map((each, index) => <Anchor onClick={each.onClick} key={index} to={each.to} className='text-white text-center font-poppins text-[15px] font-semibold leading-6 mt-[10px] w-[382px] rounded-lg cursor-pointer py-3 hover:bg-white hover:text-orange-500'>{each.title}</Anchor>)}
            </div>
        </div>
    )
}