...
import companyActions from "../store/actions/companies";



export default function AdminCompany() {
  const { saveCompanies, updateActive } = companyActions
  const dispatch = useDispatch()
  const activeTrue = useSelector(store => store.companies.companies.active_true)
  console.log(activeTrue)
  const activeFalse = useSelector(store => store.companies.companies.active_false)
  ...
  return (
    <div className="flex justify-center items-center w-[90%]">
      <table className="-mt-[5.1px] flex flex-col items-center justify-center rounded-2xl w-[80%] ">
        <tbody className="border-t-2 border-[#FF5722] rounded-b-lg w-full">
          {activeTrue?.map((item, index) => (
            // console.log(item)
            <tr key={index} className="w-full ">
              <th className="border-b-2 border-slate-400 w-1/5"><img src={adminCompany} className="ms-7 w-[19.5px] h-[21]" alt="" /></th>
